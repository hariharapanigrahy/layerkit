...
5. Leave a TODO only when customer policy or datalayer support is missing.
6. Record a redacted memory note:

```bash
layerkit memory append --type privacy --title "privacy review <vendor>" --vendor <vendor> --body-file ./privacy-digest.md
```

7. Run the client package tests that cover privacy behavior.
